...
import { setItem } from '../utils/localStorageMethods'
...
export const ConfigProvider = ({ children }) => {
   ...
   useQuery(SETTINGS_QUERY, {
      ...
      onCompleted: data => {
         if (data.settings.length > 0) {
            dispatch({
               type: 'SET_BRANDID',
               payload: { id: data.settings[0].brandId },
            })
            dispatch({
               type: 'SET_SETTINGS',
            ...
            dispatch({
               type: 'SET_APP_CONFIG',
               payload: data?.settings?.find(
                  setting => setting?.meta?.identifier === 'app-config'
               )?.value,
            })
            setIsLoading(false)
         }
      ...
      onError: error => {
         console.error('SETTINGS_QUERY', error)
      },
   })

   useQuery(GET_BRAND_LOCATION, {
      skip: !state.brand?.id || !state.locationId,
      variables: {
         where: {
            brandId: {
               _eq: state.brand?.id,
   ...
   React.useEffect(() => {
      // for app there is only one order interface App Ordering
      ;(async function () {
         const setData = await setItem('orderInterfaceType', 'App Ordering')
   ...
   // insert device info
   const [insertDeviceInfo] = useMutation(INSERT_DEVICE_INFO)

   // set device info
   React.useEffect(() => {
      ;(async function () {
         const isNotificationTokenSet = await AsyncStorage.getItem(
            'isNotificationTokenSet'
         )
         if (isNotificationTokenSet !== 'true') {
               ...
               insertDeviceInfo({
                  variables: {
                     object: {
                        ...
                        deviceType: Device.DeviceType[deviceTypeId],
                  ...
                  onError: err => {
                     console.error('insert device info error', err)
                  },
               })
            })
         }
      ...
      responseListener.current =
         Notifications.addNotificationResponseReceivedListener(response => {
            if (!isEmpty(response?.notification?.request?.content?.data)) {
               ...
               if (data.screen == 'OrderTracking') {
                  navigation.navigate('OrderTracking', {
                     cartId: data.cartId,
                  })
               } else if (data.screen == 'OrderDetail') {
...
export const useConfig = (globalType = '') => {
   ...
   const hasConfig = React.useCallback(
      (identifier = '', localType = '') => {
         ...
         if (identifier && type) {
            const identifierValue = state.settings.find(
               setting =>
                  setting.meta.type === type &&
                  setting.meta.identifier == identifier
            )?.value
